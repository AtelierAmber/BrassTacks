...
if mods["underwater-pipes"] and parts.experimental then
  rm.RemoveIngredient("underwater-pipe", "steel-plate", 999, 999)
  rm.AddIngredient("underwater-pipe", "hardened-hull", 3, 3)
end

if mods["IntermodalContainers"] then
  if parts.experimental then
    rm.AddIngredient("ic-container", "articulated-mechanism", 1, 1)
    rm.AddIngredient("ic-containerization-machine-2", "gearbox", 2, 2)
    rm.AddIngredient("ic-containerization-machine-3", "advanced-gearbox", 2, 2)
    rm.AddIngredient("ic-containerization-machine-3", "complex-joint", 10, 10)
  else
    rm.AddIngredient("ic-containerization-machine-3", "bearing", 10, 10)
  end
end
...
if mods["cargo-ships"] and parts.experimental then
  ...
  rm.AddIngredient("cargo_ship", "flywheel", 5, 5)
  rm.AddIngredient("oil_tanker", "flywheel", 5, 5)

  if not parts.nickelExperiment then
    rm.AddIngredient("bridge_base", "articulated-mechanism", 30, 30)
  end
end

if mods["Mini_Trains"] and parts.experimental then
  rm.AddIngredient("mini-locomotive", "flywheel", 1, 1)
end
...
if mods["ch-concentrated-solar"] then
  if parts.experimental then
    rm.ReplaceIngredient("chcs-heliostat-mirror", "steel-plate", "complex-joint", 1, 1)
    ...
    rm.AddIngredient("chcs-solar-laser-tower", "complex-joint", 5, 5)
    if parts.gyroscope then
      rm.AddIngredient("chcs-solar-laser-tower", parts.gyroscope, 1, 1)
    end
  else
    rm.AddIngredient("chcs-solar-laser-tower", "bearing", 5, 5)
  end
end
